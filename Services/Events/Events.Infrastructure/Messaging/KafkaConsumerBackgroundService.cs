    ...
    : BackgroundService
{
    ...
    private async Task Consume(CancellationToken stoppingToken)
    {
        ...
        logger.LogInformation("Kafka Consumer started for topic: {Topic}", KafkaTopics.BookingConfirmed);
        try
        {
    ...
    {
        // Логика отправки в DLT (как в предыдущих ответах)
        logger.LogError(exception, "Message {Key} failed all retries. Moving to DLT.", result.Message.Key);
        var dltTopicName = KafkaTopics.BookingConfirmedDlt;
