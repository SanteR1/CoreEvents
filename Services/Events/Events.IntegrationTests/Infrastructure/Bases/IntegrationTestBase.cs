...
public abstract class IntegrationTestBase<TFactory> : IAsyncLifetime where TFactory : IntegrationTestFactory
{
    ...
    internal Task ExecuteDbContextAsync(Func<EventsDbContext, Task> action)
        => ExecuteScopeAsync(sp => action(sp.GetRequiredService<EventsDbContext>()));
    ...
    internal Task<T> ExecuteDbContextAsync<T>(Func<EventsDbContext, Task<T>> action)
        => ExecuteScopeAsync(sp => action(sp.GetRequiredService<EventsDbContext>()));
