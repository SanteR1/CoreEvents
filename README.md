...
Схема БД создаётся автоматически при запуске через метод `EnsureCreated` — EF Core автоматически создаст таблицы при первом запуске, если их ещё нет
...
В тестах используется InMemory-провайдер EF
