...
| **POST** | `/events/{id}/book` | Создание брони для события | `id` (GUID события) | 202, 404 |
...
- фоновая обработка периодически вызывает метод `GetBookingForProcessing` сервиса `BookingService` который проверяет наличие очереди в хранилище бронирований `ConcurrentQueue<Guid>`, в него добавляются все брони при создании со статусом `Pending`;
- для каждой необработанной брони в очереди выполняется обработка;
- после обработки бронь переводится в статус `Confirmed`;
...
2. Используя `ID` события, создаем бронь
3. Используя из ответа полученный адрес `Location` проверяем статус брони или по `ID` события
4. Сначала будет статус `Pending`
5. Повторяем запрос полученный из `Location` спустя 3 секунды
6. Статус меняется на `Confirmed` или `Rejected`в зависимости от результа обработки
7. После смены статуса, заполняется поле `ProcessedAt` текущим временем завершения обработки
...
| Поле | Тип | Обязательно | Описание | Валидация |
| :--- | :--- | :---: | :--- | :--- |
| **EventId** | `GUID` | ✅ Да | ID события для которого создается бронь. | `Required` |
