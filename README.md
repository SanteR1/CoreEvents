...
Современный RESTful API для управления событиями, построенный на базе **ASP.NET Core 10.0**. Проект демонстрирует использование многослойной архитектуры, паттерна "Репозиторий" и централизованных ошибок.
...
Все команды выполняются из корневой папки проекта (где находится файл решения `.sln` или проектный файл `.csproj`):
...
- для фоновой обработки очереди используется `Task`, каждая задача работает со своим экземпляром контекста данных;
- фоновая обработка периодически (каждые 10 секунд) опрашивает БД бронирования со статусом `Pending` и вызывает метод `ProcessBookingAsync`;
...
- обновлённая бронь сохраняется в Базе Данных;
