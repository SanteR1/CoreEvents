...
- **Аутентификация и авторизация (Auth & Sessions)**:
  ...
  - Безопасная No-JWT архитектура на клиенте: сессии обслуживаются через HttpOnly Cookies (`credentials: 'include'`), состояние сессии хранится в оперативной памяти (`sessionStore`) и синхронизируется при старте приложения (`GET /v1/Users/me`).
  - Фоновый перехватчик `401 Unauthorized` для бесшовного обновления сессии (Silent Refresh через `POST /v1/Auth/refresh`).
  - Клиентская ролевая модель (RBAC): `requireAdminLoader` и условный рендеринг элементов управления событиями и бронированиями (`Admin` vs `User`).
  - Защищённые маршруты (`requireAuthLoader`, `requireAdminLoader`, `anonymousOnlyLoader`) с поддержкой безопасного возврата пользователя на исходную страницу (`returnUrl`).
...
| :------------- | :----------------------------------------------------------------------------- | :------------------------------- |
| `VITE_API_URL` | Базовый URL API Gateway (единая точка входа для всех запросов к микросервисам) | `http://localhost:5000`          |
...
- По умолчанию все запросы маршрутизируются через шлюз на `http://localhost:5000` (пути версий `/v1/...` определены в OpenAPI-спецификациях клиентов).
...
- **В браузере пользователя**: запросы к API отправляются на единый порт шлюза **`http://localhost:5000`**, который проброшен из Docker на хост-машину.
...
| **Количество тестов** |   **459 / 459**   |       —        | ✅ **100% passed** |
...
- [x] **Подключение к API Gateway**: Перевод клиентов `openapi-fetch` на единый базовый URL шлюза (`VITE_API_URL=http://localhost:5000`) с сохранением гибких fallback-адресов.
- [x] **Этап 1. Безопасные сессии и ролевая модель (RBAC)**:
