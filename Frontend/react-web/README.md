...
- [ ] **Этап 1. Интеграция с YARP Gateway, безопасные сессии и ролевая модель (RBAC)**:
  - _Архитектурная зависимость_: Требует готовности шлюза YARP и эндпоинта `GET /api/users/me` на бэкенде (см. [Дорожная карта системы в корневом README.md](../../README.md#-дорожная-карта-развития-системы-system-roadmap)).
  - **Безопасность сессий (No-JWT Architecture)**:
    - Полный отказ от хранения JWT в `localStorage` и декодирования токенов в JS (устранение уязвимости к XSS-атакам).
    - Перевод клиентов `openapi-fetch` на единый базовый URL шлюза (`VITE_API_GATEWAY_URL` или относительный `/api`) с параметром `credentials: 'include'` (куки передаются браузером автоматически).
  - **Управление сессией пользователя**:
    - Инициализация сессии через `GET /api/users/me` при старте приложения (Root Loader / AuthProvider).
